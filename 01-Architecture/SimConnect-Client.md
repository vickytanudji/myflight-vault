...
4. **No timeout on the underlying wait** — `python-SimConnect`'s `SimConnect()` constructor has an unbounded `while self.ok is False: pass`. If MSFS accepts a handle but never replies, `connect()` hangs **silently forever**. A watchdog timeout was added to the *diagnostic probe script* but not yet to production `client.py` — **flagged as the next brief to write.**
