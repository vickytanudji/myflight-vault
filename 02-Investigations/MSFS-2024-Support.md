...
## Decision
**Confirmed, dual-support formalized.** No sim-version detection/branching code added anywhere — genuinely not needed, since the SimConnect surface is identical for everything this project reads. Docs updated (`CLAUDE.md` / `docs/sdk/simconnect-variables.md`) to state both versions are supported.

## Still Owed
- A connection **watchdog timeout** in production `client.py` (the smoke test's own diagnostic script added one; production code still has the unbounded wait) — next brief to write.
- MSFS 2024 cold-and-dark camera-state value for [[Flight-Load-Gate]] — never tested.
