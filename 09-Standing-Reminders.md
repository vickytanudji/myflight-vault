...
## Awaiting Live Retest Only (SimConnect connection fix now confirmed live)
...
- [ ] **Full `core.main` connection-robustness retest** (beyond the one-liner script, which passed 3/3) — with MSFS closed then launched then closed/reopened, specifically test a shutdown right after a successful connect (before `start_polling()`), to exercise the never-polled-handle fix from earlier. The core `timerThread` cleanup bug itself is now confirmed fixed (2026-09-23, 3 consecutive clean attempts) — this remaining check is about the OTHER sub-fixes in the same area, not this specific bug.
